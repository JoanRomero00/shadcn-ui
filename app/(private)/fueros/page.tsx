...
import { apiFuerosGetCollection, type Fuero } from "@/lib/api"
    ...
    try {
      ...
      if (error) {
        if (response?.status === 401) {
          ...
          setErrorMsg(`Error ${response?.status || "desconocido"}: ${response?.statusText || "No se pudieron obtener los fueros"}`)
        }
        return
      }
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-[160px] rounded-lg">
                ...
                <DropdownMenuItem
                  ...
                  onClick={() =>
                    toast.info(`Detalle de fuero: ${row.original.descripcion}`)
                  }
                >
                  Ver Detalles
                </DropdownMenuItem>
                ...
                <DropdownMenuItem
                  ...
                  onClick={() =>
                    toast.error("No cuenta con permisos para eliminar fueros.")
                  }
                >
      ...
      <SiteHeader
        ...
        action={
          <div className="flex items-center gap-2">
            ...
            <Button
              onClick={() =>
                toast.info("Formulario de creación de fueros en desarrollo.")
              }
      ...
      <div className="flex flex-1 flex-col gap-4 p-4 md:p-5 lg:p-6 max-w-[1600px] w-full mx-auto overflow-hidden">
        ...
        {errorMsg && (
          <div className="flex items-center gap-3 bg-destructive/15 border border-destructive/25 text-destructive p-4 rounded-xl text-sm max-w-lg mx-auto my-6 shadow-sm w-full">
            ...
            <Button size="sm" variant="outline" className="border-destructive/30 hover:bg-destructive/10 text-destructive font-semibold" onClick={fetchFueros}>
              Reintentar
            </Button>
          </div>
        )}
